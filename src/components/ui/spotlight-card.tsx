...
const GlowCard: React.FC<GlowCardProps> = ({
  ...
  const getInlineStyles = (): React.CSSProperties & Record<string, unknown> => {
    const baseStyles: React.CSSProperties & Record<string, unknown> = {
      ...
      '--radius': '14',
      '--border': '2',
      '--backdrop': 'rgba(10, 26, 10, 0.6)',
      '--backup-border': 'rgba(74, 222, 128, 0.15)',
      '--size': '250',
      '--outer': '1',
      ...
      backgroundImage: `radial-gradient(
        ...
        calc(var(--x, 0) * 1px)
        calc(var(--y, 0) * 1px),
        hsl(var(--hue, 120) calc(var(--saturation, 80) * 1%) calc(var(--lightness, 60) * 1%) / var(--bg-spot-opacity, 0.08)), transparent
      )`,
  ...
  return (
    <>
      ...
      <div
        ...
        className={`
          ${getSizeClasses()}
          rounded-2xl relative grid shadow-[0_1rem_2rem_-1rem_black]
